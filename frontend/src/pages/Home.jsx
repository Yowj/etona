...
const Home = () => {
  ...
  useEffect(() => {
    try {
      setLoading(true);
      axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/webnovel`).then((response) => {
            ...
            <aside className="w-1/4 bg-white p-6">
              ...
              <ul className="space-y-2">
                {allCategories.map((category) => (
                  <li key={category}>
                    <button
                      className="text-gray-700 cursor-pointer bold hover:text-gray-900 focus:outline-none"
